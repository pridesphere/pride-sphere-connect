...
const StartNewChatModal: React.FC<StartNewChatModalProps> = ({ isOpen, onClose, onChatCreated }) => {
  const { user } = useAuth();
    ...
    const fetchFriends = async () => {
      try {
        ...
        const friends: User[] =
          profiles
            ?.filter(
              (profile) => !searchQuery || profile.display_name?.toLowerCase().includes(searchQuery.toLowerCase()),
            )
            .map((profile) => ({
              id: profile.user_id,
  ...
  const handleUserSelection = (userId: string) => {
    setSelectedUsers((prev) => {
      ...
      const newSelection = isSelected ? prev.filter((id) => id !== userId) : [...prev, userId];
  ...
  const createConversation = async () => {
    // MODIFICATION START: Use the user from the AuthProvider directly.
    if (!user) {
      toast({
        title: "Authentication Error",
        description: "You must be logged in to create a chat. Please log in again.",
        variant: "destructive",
      });
      setLoading(false);
      return;
    ...
    if (selectedUsers.length === 0) {
      toast({
        title: "Error",
        description: "Please select a user to start a chat.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const conversationData = {
        ...
        created_by: user.id, // Use the reliable user ID from the auth context
      };
      ...
      const { error: participantError } = await supabase.from("conversation_participants").insert(participantInserts);
      ...
      toast({
        title: "Success",
        description: `笨ｨ ${isGroupChat ? "Group chat" : "Chat"} created successfully!`,
      });
  ...
  const filteredUsers = availableUsers.filter((u) => u.display_name.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
              ...
              filteredUsers.map((u) => (
                <div
                  ...
                  className={`flex items-center space-x-3 p-3 rounded-lg cursor-pointer transition-colors ${
                    selectedUsers.includes(u.id) ? "bg-primary/10 border border-primary/20" : "hover:bg-muted/50"
                  }`}
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {u.pronouns && <p className="text-xs text-muted-foreground">{u.pronouns}</p>}
                  </div>
                </div>
              ))
            )}
          ...
          <div className="flex space-x-2 pt-4">
            ...
            <Button onClick={createConversation} disabled={selectedUsers.length === 0 || loading} className="flex-1">
              {loading ? (
                <>Creating...</>
              ) : (
                <>
